"""
Base module for the MOSESDMD: `fit` method must be implemented in inherited classes
The same as dmdbase, but with MOSES SVD
"""
...
class MOSESDMDBase(object):
    ...
    def _compute_tlsq(X, Y, tlsq_rank):
        ...
        msvd = MOSESSVD(rank=tlsq_rank)
        V = msvd.iterated_svd(np.append(X, Y, axis=0), b=tlsq_rank+1)[-1].conj().T
    ...
    def _compute_svd(X, svd_rank):
        ...
        msvd = MOSESSVD(rank=svd_rank)
        U, s, V = msvd.iterated_svd(X, b=svd_rank+1)
